...
class NetworkAPI: NSObject {
    ...
    static func sendRequest(_ pin: Pin, completionHandlerForNetworkRequest: @escaping (_ photosDict: [[String:AnyObject]]?, _ success: Bool, _ error: NSError?) -> Void) {
        ...
        let URLParams = [
            ...
            "extras": "url_m",
            "nojsoncallback": "1",
            "api_key": "165259f4a4b6ebebe4a513a270a7b544",
        ...
        let task: URLSessionDataTask = session.dataTask(with: request as URLRequest, completionHandler: { (data: Data?, response: URLResponse?, error: Error?) in
            ...
            guard error == nil else {
                sendError("Flickr API returned an error: \(String(describing: error))", code: Flickr.networkError)
                return
            }
            ...
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else {
                ...
                sendError("Flickr returned a status code outside the success range: \(String(describing: theStatusCode))", code: Flickr.requestError)
                return
            }
